...
interface TodoDto {
  ...
  focusTime: number;
  order: number | null; // TODO : 리스트에는 done 된 항목에 대해서는 안불러 줄태니까 null은 필요 없을지도 모른다.
}

interface ITodoListModalProps {
  todos: Map<string, TodoDto[]>;
}
...
const listRenderer = (mapTodo: Map<string, TodoDto[]>) => {
  const dateList = Array.from(mapTodo.keys());
  const todoList = Array.from(mapTodo.values());

  const dndList = dateList.map((date, idx) => (
...
const TodoListModal = ({ todos }: ITodoListModalProps) => {
  // const [mapTodo, setMapTodo] = useState<Map<string, TodoDto[]>>(groupedStubs);
  const [mapTodo, setMapTodo] = useState<Map<string, TodoDto[]>>(todos);
